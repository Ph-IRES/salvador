...
data_vegan_x17 <- data_vegan %>%
  filter(row_number() != 17)

data_vegan_x17.env <- data_vegan.env %>%
  filter(op_code != "CAG_017") # very different from all other bruvs


dbrda <- 
  dbrda(
    formula = data_vegan_x17 ~ study_locations * habitat,
    data = data_vegan_x17.env,
    distance = "bray"
  )
print(dbrda)
plot(dbrda)
...
dbrda_summary$concont

# # use envfit to generate species loading vectors
# # https://stackoverflow.com/questions/14711470/plotting-envfit-vectors-vegan-package-in-ggplot2
# dbrda_species_vectors <- 
#   envfit(dbrda$points, 
#          data_vegan_x17,
#          perm=1000)
# 
...
#### dbRDA Plot of Fish Assemblage at Different Shallow and Deep Reefs at Cagayancillo and Tubbataha ####
habitatcolors <- c("#F08080",
                   "#6FAFC6")
habitatlabels <- c("Shallow",
                   "Mesophotic")
...
ggdbrda_plot_wellipses_wovectors <- 
  ggdbrda %>%
  ggplot(aes(x = db_rda1,
             y = db_rda2,
             color = habitat,
  ...
  scale_shape_manual(values = c(16,2)) +
  stat_ellipse(aes(group = studylocation_habitat,
                   lty=factor(study_locations)), show.legend = FALSE) +
  scale_linetype_manual(values=c(1,2,1,2)) +
  theme_classic() +
  xlab("dbRDA 1") +
  ylab("dbRDA 2") +
...
ggsave("dbRDAfishassemblagewellipses.png",
       ggord_plot_wellipses_wovectors,
       height = 5,
       width = 7,
       units = "in")

legend <- get_legend(ggord_plot_wellipses_wovectors)


pdf("nmds_dbrda_ordinations.pdf", paper = "letter", width = 8)
gridExtra::grid.arrange(
  ggord_plot_wellipses_wovectors,
  ggdbrda_plot_wellipses_wovectors,
  legend,
  widths = c(10,10.4,2),
  heights = c(10,1,10.4,2),
  respect = TRUE,
  ncol = 3
)
dev.off()
